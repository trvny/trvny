...
> **In claude.ai chat.** Doc retrieval (web fetch) and code authoring work normally. The `node_modules/wrangler/config-schema.json` references assume a local checkout with deps installed; in chat there's no repo on disk, so read schema/config from the live docs (always works). A sandbox `git clone` + `npm install` is only an option for **public** repos — the sandbox has no GitHub auth; for a private repo, use the live docs or the github connector rather than pasting a token. Deploying or running against the user's Cloudflare account isn't possible here (no account auth) — write the commands/config for the user to run. See the `wrangler` skill for the CLI specifics.
