---
name: skill-builder
description: Automatically detect source types and build AI skills using the Skill Seekers MCP server. Requires the Skill Seekers MCP connector to be connected — without it, none of these tools are available. Use when the user wants to create skills from documentation, repos, PDFs, videos, or other knowledge sources.
---

# Skill Builder

> **Prerequisite.** This skill drives the **Skill Seekers MCP server** (35 tools). It only works if that connector is connected. If those tools aren't available, don't improvise around them — tell the user this skill needs the Skill Seekers MCP connected (Settings → Connectors), and offer the built-in `skill-creator` skill as the connector-free alternative for authoring a skill by hand.
