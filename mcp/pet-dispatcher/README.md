...
For the control plane, create the Queue, set `CONTROL_PLANE_TOKEN` and `TASK_SIGNING_SECRET` as Worker secrets, and store the private connector value as repository secret `PET_DISPATCHER_MCP_CONNECTOR_TOKEN`; the deploy workflow syncs it into Worker secret `MCP_CONNECTOR_TOKEN` before deploying `control-plane/wrangler.jsonc`. Then enable the HTTP pull consumer. Botek should bind to the named `TelegramAssistantEntrypoint` through a same-account Cloudflare Service Binding. That RPC surface is the scoped capability: it can delegate only `inspect`/`code` agent tasks with default capabilities and `network=none`, read task state, and cancel tasks. It exposes no direct-tool or publish/custom-capability path and needs no shared bearer secret. `npm run control:check` validates the Worker bundle without deploying it.
