...
`/mcp` is a stateless Streamable HTTP MCP endpoint. Operator clients may keep using `Authorization: Bearer <CONTROL_PLANE_TOKEN>`. Custom-connector UIs that offer a URL but no arbitrary header can instead use `https://<worker>/mcp/<MCP_CONNECTOR_TOKEN>` with connector auth set to **None**. `MCP_CONNECTOR_TOKEN` is a separate secret accepted only on the MCP route, never on `/v1/*`; do not reuse `CONTROL_PLANE_TOKEN` in the URL. Invocation logs are disabled so the secret path is not persisted by Worker logging. The endpoint exposes only `pet_meta`, `pet_delegate`, `pet_direct`, `pet_task_get` and `pet_task_cancel`; those tools reuse the existing task schemas, assistant guard and direct-tool capability derivation instead of defining a second authority model. `pet_delegate` and `pet_direct` can wait briefly for a terminal result and otherwise return the task id for later polling. Optional idempotency keys make client retries resolve to the same task. A public multi-user app still needs proper OAuth/pairing rather than this private secret-URL compatibility mode.
MCP `initialize` advertises the display title **Pet Dispatcher**, a concise description, the repository URL and the public `https://pet-dispatcher-control.travny.workers.dev/icon.png` icon. All five tools publish `outputSchema` plus structured content so clients can render and reason about results without reverse-engineering text. `assets/pet-dispatcher.svg` is the maintained icon source; `npm run build:icon` regenerates both `assets/pet-dispatcher.png` and the inlined Worker module.
...
For the control plane, create the Queue, set `CONTROL_PLANE_TOKEN` and `TASK_SIGNING_SECRET` as Worker secrets, and store the private connector value as repository secret `PET_DISPATCHER_MCP_CONNECTOR_TOKEN`. The shared `.github/scripts/sync-cloudflare-worker-secret.sh` helper is used both by the deploy workflow and the central `Sync Worker credentials` workflow to mirror it into Worker secret `MCP_CONNECTOR_TOKEN` before deployment or on manual resync. Then enable the HTTP pull consumer. Botek should bind to the named `TelegramAssistantEntrypoint` through a same-account Cloudflare Service Binding. That RPC surface is the scoped capability: it can delegate only `inspect`/`code` agent tasks with default capabilities and `network=none`, read task state, and cancel tasks. It exposes no direct-tool or publish/custom-capability path and needs no shared bearer secret. `npm run control:check` validates the Worker bundle without deploying it.
