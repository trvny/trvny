...
export const remoteCapabilitySchema = z.enum([
  "workspace.read", "workspace.write", "process.exec", "git.read", "git.commit", "tests.run", "network.fetch",
  "adb.inspect", "adb.install", "github.pr.create",
]);
...
export const remoteNetworkSchema = z.object({
  ...
  if (value.mode === "none" && value.profile) ctx.addIssue({ code: "custom", message: "network profile is invalid when mode is none" });
  if (value.mode === "brokered" && !value.profile) ctx.addIssue({ code: "custom", message: "brokered network mode requires a profile" });
});
...
export const signedTaskEnvelopeSchema = z.object({ envelope: taskEnvelopeSchema, signature: z.string().regex(/^[A-Za-z0-9_-]{43}$/u) });
...
function fromBase64Url(value: string): ArrayBuffer {
  ...
  const binary = atob(base64); return Uint8Array.from(binary, (char) => char.charCodeAt(0)).buffer as ArrayBuffer;
}
...
export async function signEnvelope(envelope: TaskEnvelope, secret: string): Promise<SignedTaskEnvelope> {
  ...
  const signature = await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(canonicalJson(envelope)));
...
export async function verifyEnvelope(signed: SignedTaskEnvelope, secret: string): Promise<boolean> {
  const key = await hmacKey(secret);
  return crypto.subtle.verify("HMAC", key, fromBase64Url(signed.signature), new TextEncoder().encode(canonicalJson(signed.envelope)));
}
...
export async function signWorkerRequest(secret: string, method: string, path: string, timestamp: string, nonce: string, body: string): Promise<string> {
  const key = await hmacKey(secret);
  const signature = await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(requestPayload(method, path, timestamp, nonce, body)));
  return base64Url(new Uint8Array(signature));
}
export async function verifyWorkerRequest(secret: string, signature: string, method: string, path: string, timestamp: string, nonce: string, body: string): Promise<boolean> {
...
export function validateEnvelopeFreshness(signed: SignedTaskEnvelope, expectedDeviceId: string, now = Date.now()): void {
  ...
  if (envelope.expiresAt - envelope.issuedAt > 24 * 60 * 60_000) throw new Error("remote task envelope lifetime exceeds 24 hours");
}
