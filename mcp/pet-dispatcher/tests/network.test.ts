...
test("URL guard allows only profiled HTTPS destinations", () => {
  const githubHosts = config.networkProfiles.github?.hosts;
  assert.ok(githubHosts);
  assert.equal(assertAllowedUrl("https://api.github.com/repos/trvny/trvny", githubHosts).hostname, "api.github.com");
  assert.equal(assertAllowedUrl("https://raw.githubusercontent.com/trvny/trvny/main/README.md", githubHosts).hostname, "raw.githubusercontent.com");
  assert.throws(() => assertAllowedUrl("http://api.github.com", githubHosts), /HTTPS only/);
  assert.throws(() => assertAllowedUrl("https://evil.example", githubHosts), /outside/);
  assert.throws(() => assertAllowedUrl("https://127.0.0.1", githubHosts), /IP-literal/);
  assert.throws(() => assertAllowedUrl("https://[::1]", githubHosts), /IP-literal/);
  assert.throws(() => assertAllowedUrl("https://user:pass@api.github.com", githubHosts), /credentials/);
});
...
test("broker validates every redirect and truncates response bodies", async () => {
  ...
  const fakeFetch = ((input: string | URL | Request) => {
    ...
    if (url.pathname === "/start") return Promise.resolve(new Response(null, { status: 302, headers: { Location: "/final" } }));
    return Promise.resolve(new Response("1234567890", { status: 200, headers: { "Content-Type": "text/plain", "X-Secret": "nope" } }));
  }) as typeof fetch;
...
test("broker rejects unsafe caller-supplied headers", async () => {
  const broker = new NetworkBroker(config, (() => Promise.resolve(new Response("ok"))) as typeof fetch);
...
test("broker fails closed on redirect to an unprofiled host", async () => {
  const fakeFetch = (() => Promise.resolve(new Response(null, {
    status: 302,
