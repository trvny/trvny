// Template for build_test.py - the class-body placeholder further down is replaced
// with the patched Bh class extracted from a real main.js, then run under node with
// no dependencies.

// ---- stubs for the surrounding bundle ----
class Mp { async checkConnectCommonOps() { return true } }
const r = (thisArg, _a, _P, generator) => new Promise((resolve, reject) => {
  function step(f, v) { let res; try { res = f.call(gen, v) } catch (e) { return reject(e) }
    if (res.done) return resolve(res.value)
    Promise.resolve(res.value).then(x => step(gen.next, x), x => step(gen.throw, x)) }
  const gen = generator.call(thisArg)
  step(gen.next, undefined)
})
const I = "application/octet-stream"
const kh = "application/vnd.google-apps.folder"
const dh = { contentType: () => "text/markdown", lookup: () => "text/markdown" }
const Vp = (t) => new Date(t).toISOString()
const zp = (k) => { const parts = k.split("/").slice(0, -1); const out = []; let cur = ""
  for (const p of parts) { cur += p + "/"; out.push(cur.slice(0, -1)) } return out }
const Th = (f, pid, ppath) => ({ keyRaw: (ppath || "") + f.name, id: f.id, parentID: pid, parentIDPath: ppath, isFolder: f.mimeType === kh })
const th = (size, chunk) => { const out = []; for (let st = 0; st < size; st += chunk) out.push({ start: st, end: Math.min(st + chunk, size) - 1 }); return out }
class vh { constructor() {} on() {} add(f) { return f() } async onIdle() {} pause() {} clear() {} }
const Eh = async () => ({ access_token: "tok", expires_in: 3600 })
...
const mkfs = () => { const fs = new Bh({ accessToken: "tok", refreshToken: "ref", accessTokenExpiresAtTimeMs: Date.now() + 9e6 }, "Vault", async () => {})
  fs.baseDirID = "BASE"; fs.vaultFolderExists = true; return fs }
