---
name: cmd-tvpi-add-channel
description: Add a new live channel to the travino/tvpi IPTV project end-to-end — pick the right source type (TVP API, direct HLS manifest, or YouTube live), add the channel to both worker/src/index.ts and generate.py so the Worker and the raw mirror agree, and verify it resolves. Use whenever the user wants to add/include a channel, mentions "add TVP X", "add this stream", "put channel Y in the playlist", gives a vod.tvp.pl live URL or an .m3u8 URL, or points at the tvpi repo wanting a new entry.
---
...
- `streams/{slug}.m3u` is regenerated on the next `*/15` cron; to publish immediately run `gh workflow run refresh.yml -R travino/tvpi` (or the Actions tab). The `git add streams/` glob commits the new file with no workflow change.
