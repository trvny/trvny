...
> **In claude.ai chat.** The generation scripts run fine in the bash sandbox (`pip install --break-system-packages pillow ...` as needed). Where this skill says **"AskUserQuestion tool,"** use this chat's question tool (`ask_user_input_v0`) — same idea: tappable options instead of plain-text questions. Deliver the generated files with `present_files` so the user can download them.
...
✅ **Professional**: Clean, consistent visual selection UI
