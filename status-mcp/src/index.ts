...
const UA = "status-mcp (+https://github.com/travino/travino)";
...
async function listFeedFiles(): Promise<Array<{ name: string; size: number }> | null> {
  try {
    const res = await fetch(`${FEEDS_API}/contents/feeds?ref=main`, {
...
async function checkFeeds(): Promise<ProjectResult> {
  try {
    const [yamlRes, files, pipeline] = await Promise.all([
      fetch(`${FEEDS_RAW}/feeds.yaml`, { signal: timeout(FETCH_TIMEOUT_MS) }),
      listFeedFiles(),
