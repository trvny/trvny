...
const stage = document.querySelector(".radio-now-playing");
const image = stage?.querySelector("img");
const fallback = stage?.querySelector(".radio-artwork-fallback");
const title = stage?.querySelector(".radio-artwork-copy strong");
const meta = stage?.querySelector(".radio-artwork-copy span");
let timer = null;
let generation = 0;
let currentUrl = "";
let stationArtwork = "";
...
function applyMetadata(data) {
  ...
  if (meta) meta.textContent = [artist, album].filter(Boolean).join(" · ") || data.station || stationTitle || "Stream audio";
  setArtwork(artwork);
...
async function poll(expectedGeneration) {
  ...
  try {
    ...
    applyMetadata(data);
    timer = setTimeout(() => poll(expectedGeneration), Math.max(10, Number(data.refreshAfter) || 20) * 1000);
  } catch {
    if (expectedGeneration === generation) timer = setTimeout(() => poll(expectedGeneration), 45_000);
  }
}
...
function start() {
  queueMicrotask(() => {
    ...
    clearTimer();
    generation += 1;
    if (!url || shell?.dataset.mode !== "audio" || !(window.streambenchBundledUrls instanceof Set) || !window.streambenchBundledUrls.has(url)) {
      currentUrl = "";
      return;
    }
    currentUrl = url;
