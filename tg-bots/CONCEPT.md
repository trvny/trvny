...
2. **Long-term memory** — explicit owner-only `/remember`, `/recall` and `/memory_status` use Engram through the shared Specialist Intelligence Worker; Botek never owns the Engram credential. Next add bounded automatic retrieval where prior context is clearly relevant. True `forget` remains pending until the canonical Engram bridge exposes deletion; do not fake deletion with tombstones.
...
5. **Scheduler and proactive assistance** — completed Botek task notifications and bounded one-shot relative reminders are implemented on the shared minute cron. Next add recurring briefings, condition watches and important RSS/CI/service alerts while avoiding noisy low-value notifications.
