...
function validTurn(value: unknown): value is Omit<TelegramConversationTurn, "createdAt"> {
...
export class TelegramConversationMemory {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === "/append") {
      ...
      const turn: TelegramConversationTurn = { ...payload, createdAt: new Date().toISOString() };
      const turns = [...(current?.turns ?? []), turn].slice(-MAX_STORED_TURNS);
      await this.state.storage.put(STATE_KEY, { turns, updatedAt: turn.createdAt } satisfies ConversationState);
